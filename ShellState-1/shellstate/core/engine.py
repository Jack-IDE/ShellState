...
class TerminalApp:
    ...
    def _panel_plain(self, lines: List[str], width: int, height: int) -> List[str]:
        inner_w = max(1, width)
        inner_h = max(1, height)
        padded = list(lines[:inner_h]) + [""] * max(0, inner_h - len(lines))
        return [self._fit_panel_line(line, inner_w) for line in padded]
    ...
    def _render_canvas_panels(self, screen: ScreenDict, available_height: Optional[int] = None) -> List[str]:
        ...
        only_min_floor = panel_min_height
        if main_spec is not None and info_spec is None:
            try:
                only_min_floor = int(self._screen_value(screen, "main_panel_min_height", panel_min_height) or panel_min_height)
            except Exception:
                only_min_floor = panel_min_height
            only_min_floor = max(5, only_min_floor)

        borderless_main_panel = bool(self._screen_value(screen, "main_panel_borderless", False)) and main_spec is not None and info_spec is None
        square_main_panel = bool(self._screen_value(screen, "main_panel_square", False)) and main_spec is not None and info_spec is None
        if square_main_panel:
            panel_height = available_height if available_height is not None else max(only_min_floor, len(only_lines) + 2)
            if panel_max_height is not None:
                panel_height = min(panel_height, panel_max_height)
            panel_height = max(only_min_floor, int(panel_height))
            if borderless_main_panel:
                inner_h = max(1, panel_height)
                panel_width = min(max(24, inner_h * 2), width - 2)
                inner_w = max(1, panel_width)
            else:
                inner_h = max(1, panel_height - 2)
                panel_width = min(max(24, (inner_h * 2) + 4), width - 2)
                inner_w = max(4, panel_width - 4)

            had_prev_w = hasattr(self, "_panel_render_inner_width")
            had_prev_h = hasattr(self, "_panel_render_inner_height")
            prev_w = getattr(self, "_panel_render_inner_width", None)
            prev_h = getattr(self, "_panel_render_inner_height", None)
            self._panel_render_inner_width = inner_w
            self._panel_render_inner_height = inner_h
            try:
                only_lines = self._capture_render_lines(main_spec)
            finally:
                if had_prev_w:
                    self._panel_render_inner_width = prev_w
                else:
                    try:
                        delattr(self, "_panel_render_inner_width")
                    except AttributeError:
                        pass
                if had_prev_h:
                    self._panel_render_inner_height = prev_h
                else:
                    try:
                        delattr(self, "_panel_render_inner_height")
                    except AttributeError:
                        pass

            box = self._panel_plain(only_lines, panel_width, panel_height) if borderless_main_panel else self._panel_box(only_lines, panel_width, panel_height, only_title)
        ...
        borderless_single_panel = bool(self._screen_value(screen, "main_panel_borderless", False)) and main_spec is not None and info_spec is None
        if fill:
            panel_width = max(24, width - 2)
        else:
        ...
        panel_height = _final_height(only_lines, min_floor=only_min_floor, fill=fill)
        box = self._panel_plain(only_lines, panel_width, panel_height) if borderless_single_panel else self._panel_box(only_lines, panel_width, panel_height, only_title)
        if panel_width >= width - 2:
            return box
        pad = " " * max(0, (width - panel_width) // 2)
        return [pad + line for line in box]
    ...
    def _build_frame(self) -> str:
        ...
        panel_below_menu = bool(screen.get("main_panel_below_menu"))

        if not panel_below_menu:
            if panel_lines:
                if lines:
                    lines.append("")
